    ...
    <div className="mx-auto max-w-xl space-y-16">
      ...
      <FadeIn delay={0.15}>
        <div className="border border-black/[0.06] bg-black p-10 text-center">
          ...
          <p className="mt-3 text-[13px] leading-[1.9] text-white/25">{styleDetails.primary.description}</p>
          {styleDetails.secondary && (
            <p className="mt-6 text-[11px] text-white/10">2순위 — {styleDetails.secondary.style}</p>
          )}
        </div>
      </FadeIn>
        ...
        <div className="mt-6 space-y-4">
          {spacePriority.slice(0, 5).map((sp, i) => (
            <div key={sp.space} className="flex items-center gap-6">
              <span className="w-4 font-mono text-[10px] text-black/10">{i + 1}</span>
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {processes.map((p) => (
            <span key={p.process} className={`px-3 py-1.5 text-[10px] tracking-[0.05em] ${
              p.priority === "필수" ? "bg-black text-white" : p.priority === "권장" ? "border border-black/10 text-black/40" : "text-black/15"
            }`}>{p.process}</span>
          ))}
        </div>
      </FadeIn>

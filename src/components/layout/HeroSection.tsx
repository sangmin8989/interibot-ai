    ...
    <div className="relative h-full w-full overflow-hidden bg-[#0f0f1a]">
      ...
      <svg viewBox="0 0 600 400" className="absolute inset-0 h-full w-full opacity-[0.06]">
        <g stroke="#D4764B" strokeWidth="0.8" fill="none" opacity="0.6">
          ...
          <line x1="80" y1="250" x2="520" y2="250" opacity="0.1"/>
        </g>
      </svg>
    ...
    <section className="relative min-h-screen overflow-hidden bg-white">
      <div className="mx-auto max-w-7xl px-6 pt-28 lg:pt-36">
        <div className="grid items-start gap-12 lg:grid-cols-12 lg:gap-8">
          ...
          >
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2, duration: 1, ease }}
              className="text-[10px] tracking-[0.5em] text-[#C9A96E]">
              INFORMED DECISIONS
            </motion.p>

            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 1.2, ease }}
            ...
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9, duration: 1, ease }}
              className="mt-8 max-w-sm text-[14px] leading-[2] text-black/35">
              121,515건의 시장 데이터가 만든 기준.
              <br />견적서를 읽는 눈을 드립니다.
            </motion.p>

            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.3, duration: 1, ease }}
              className="mt-12 flex items-center gap-8">
              <Link href="/audit" className="border-b border-black pb-1 text-[12px] font-medium tracking-[0.1em] text-black transition-all duration-500 hover:border-[#C9A96E] hover:text-[#C9A96E]">
                견적서 분석
              </Link>
              <Link href="/intevity" className="text-[12px] tracking-[0.1em] text-black/40 transition-colors duration-500 hover:text-black/70">
                성향분석
              </Link>
            </motion.div>

            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.8, duration: 1, ease }}
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2, duration: 1, ease }}
